...
#define N_MESSG   10 // the number of messages produced by each producer
...
Consumer(_int which)
{
    ...
    for (; ; ) {
      ...
      if ( write(fd, str, strlen(str)) == -1 ) {
		...
		printf("Consumer %d write failed\n",which);
	    Exit(1);
	  }
    }
}
...
ProdCons()
{
    ...
    // ....
	nfull = new Semaphore("nempty",0);
	nempty = new Semaphore("nfull",BUFF_SIZE);
