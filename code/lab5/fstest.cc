...
Append(char *from, char *to, int half)
{
    ...
    OpenFile* openFile;
    int amountRead, fileLength, 
		pastLength, pastTotalSectorSize;
    char *buffer;
...
// Figure out length of UNIX file
    ...
    pastLength = openFile->Length();
    if (half) start = pastLength / 2;
	else start = pastLength;
//    openFile->Seek(start);

	if(!fileSystem->Append(to,fileLength)){
		printf("Append: couldn't append space to filehdr of the file %s\n", to);
		fclose(fp);
		return;
	}
	delete openFile;
	openFile = fileSystem->Open(to);	//need to reload after append
	openFile->Seek(start);
...
//	openFile->WriteBack();
// There is no need to WriteBack again because Write have done it
//
//	printf("inodes have been written back\n");
	printf("Append %s to %s finished.\n",from,to);
...
NAppend(char *from, char *to)
{
    ...
    if ( (openFileTo = fileSystem->Open(to)) == NULL)
    {
		...
		if (!fileSystem->Create(to, 0)) 
		{
			printf("NAppend: couldn't create the file %s to append\n", to);
			delete openFileFrom;
			return;
		}
	...
	if(!fileSystem->Append(to,fileLength)){
		printf("NAppend: couldn't append space to filehdr of the file %s\n", to);
		delete openFileFrom;
		return;
	}
	delete openFileTo;
...
// printf("inodes have been written back\n");
	printf("NAppend %s to %s finished.",from,to);
